...
class GetAjaxFormValuesGost33259Test(TestCase):
    ...
    data_type_fl = {'type_fl': '11'}
    data_final = {'type_fl': '11', 'dn_passage': '50'}
    ...
    def test_get_dn_passage_option(self):
        ...
        response = self.client.get(self.url, self.data_type_fl)
    ...
    def test_get_pn_option(self):
        expected_data = {
            'pn_option': [
                1.0, 2.5, 6.0, 10.0, 16.0, 25.0, 40.0,
                63.0, 100.0, 160.0, 50.0, 200.0, 250.0
            ]
        }
        response = self.client.get(self.url, self.data_final)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), expected_data)
